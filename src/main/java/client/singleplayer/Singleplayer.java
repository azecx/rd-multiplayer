...
public final class Singleplayer {
    ...
    public static String winfix(String worldName) {
    ...
    public static Path worldDir(String worldName) {
        return SAVES_DIR.resolve(winfix(worldName));
    }
    ...
    public static synchronized void start(String worldName) {
        ...
        String safeName = winfix(worldName);
        Path dir = worldDir(safeName);

        try {
            Server.start(dir, 0, InetAddress.getLoopbackAddress(), false);
        } catch (IOException e) {
            System.err.println("Failed to start embedded server for world \"" + safeName + "\"");
            e.printStackTrace();
            return;
        }
